...
mburl="${MB_URL:-https://www.daozhao.com/_res/fangqiang}"
mbtmp="/tmp/mbtmp"
...
logsh "【Tools】" "下载工具箱文件..."
rm -rf ${mbtmp}/mixbox.zip > /dev/null 2>&1
if command -v curl &> /dev/null; then
	result=$(curl -w %{http_code} -skLo ${mbtmp}/mixbox.zip ${mburl}/appstore/mixbox_${model}.zip)
else
	wget-ssl -q --no-check-certificate --tries=1 --timeout=10 -O ${mbtmp}/mixbox.zip ${mburl}/appstore/mixbox_${model}.zip
	[ $? -eq 0 ] && result="200"
fi
[ "$result" != "200" ] && logsh "【Tools】" "文件下载失败！" && exit 1
logsh "【Tools】" "解压工具箱文件"
echo ${mbtmp}/mixbox.zip
unzip -n ${mbtmp}/mixbox.zip -C ${mbtmp} > /dev/null
